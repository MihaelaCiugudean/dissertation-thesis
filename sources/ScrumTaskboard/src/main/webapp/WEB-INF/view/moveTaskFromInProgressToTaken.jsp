...
<head>
<meta charset="UTF-8">
<title>Delay a task implementation</title>
...
<body background="img/kiwi.jpg" style="font-size: 20px;">
	...
	<form:form action="moveTaskFromInProgressToTaken" method="post" modelAttribute="taskToMove">
			...
			<tr>
				<td><c:forEach items="${tasksNotTaken}" var="taskNotTaken">
						<div id="draggable" 
							...
							<%-- <a href="<c: value="/move" />">Go to menu</a> --%>
     					</div>  <br> 
	...
	<c:choose>
		<c:when test="${not empty tasksInProgressForDeveloper}">
			<div align = "center">
			<table>
					<tr> <td> Select the task whose implementation you want to delay :  </td>
					</tr>
					<tr>
						<td>
						 <form:select path="id">
								<!-- <option value="">- Please choose -</option> -->
	               			    <form:options items="${tasksInProgressForDeveloper}" itemValue="id" itemLabel="description"/>
	                            <form:errors path="id"/>
					     </form:select>
				        </td>
					</tr>
					...
					<tr>
						<td><input type="submit" value="Delay task implementation" style="font-size: 14px" /></td>
					</tr>
			</table>
			</div>
		</c:when>
		...
		<c:otherwise>
			<div align = "center" style="color: red;">
				<b> You have no tasks in progress ! </b>
			</div> 
		</c:otherwise>
